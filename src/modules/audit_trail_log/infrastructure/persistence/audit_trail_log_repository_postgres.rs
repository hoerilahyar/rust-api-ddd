...
use crate::modules::audit_trail_log::domain::{
...
impl AuditTrailLogRepository for AuditTrailLogRepositoryPg {
    async fn list(
        ...
        WHERE ($1 = '' OR entity_type ILIKE $2 OR entity_id::TEXT ILIKE $2)
        ...
        let total: i64 = sqlx::query(
            r#"
        SELECT COUNT(*) AS total FROM audit_trail_logs
        WHERE ($1 = '' OR entity_type ILIKE $2 OR entity_id::TEXT ILIKE $2)
          AND ($3::INT IS NULL OR user_id = $3)
          AND ($4::VARCHAR IS NULL OR action = $4)
          AND ($5::TIMESTAMPTZ IS NULL OR created_at >= $5)
    ...
    async fn find_by_id(&self, id: i64) -> Result<Option<AuditTrailLog>, AppError> {
        let row = sqlx::query("SELECT * FROM audit_trail_logs WHERE id = $1")
            .bind(id)
