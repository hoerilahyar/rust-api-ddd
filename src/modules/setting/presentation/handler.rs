...
pub async fn get_setting(
    ...
    ensure_permission(&claims, "settings.manage")?;
    let setting = state.setting_service.get_by_key(&id).await?;
    Ok(ApiResponse::new("ok", SettingResponse::from(setting)))
}

/// `PUT /settings/:id` -- create-or-replace, so a client can introduce a
...
pub async fn upsert_setting(
    ...
    ensure_permission(&claims, "settings.manage")?;
    let setting = state.setting_service.upsert(&id, payload, claims.sub).await?;
...
pub async fn delete_setting(
    ...
    ensure_permission(&claims, "settings.manage")?;
    state.setting_service.delete(&id, claims.sub).await?;
