-- Permission for the log_audit_auth_log module (login-attempt / authentication
-- audit trail), analogous to 010_seed_audit_trail_log.sql for
-- audit_trail_log.
--
-- Without this seeder, `log_audit_auth_log::presentation::handler` guards both
-- of its routes with `ensure_permission(&claims, "log_audit_auth.read")`, but
...
-- the audit module being split into log_audit_auth_log/audit_trail_log and is
...
INSERT INTO permissions (name, description) VALUES
    ('log_audit_auth.read', 'View authentication audit history (login attempts)')
...
WHERE r.name = 'admin'
  AND p.name = 'log_audit_auth.read'
